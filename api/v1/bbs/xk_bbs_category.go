...
func (api *BbsCategoryApi) CreateBbsCategory(c *gin.Context) {
	// 1: 第一件事情就准备数据的载体
	var saveReq request2.BbsCategorySaveReq
...
func (api *BbsCategoryApi) UpdateBbsCategory(c *gin.Context) {
	// 1: 第一件事情就准备数据的载体
	var bbsReq request2.BbsCategorySaveReq
...
// 更新状态
func (api *BbsCategoryApi) UpdateBbsCategoryStatus(c *gin.Context) {
	// 1: 第一件事情就准备数据的载体
	var statusReq request2.StatusReq
...
func (api *BbsCategoryApi) GetBbsCategory(c *gin.Context) {
...
//	@Success	200		{object}	response.Response{data=exampleRes.ExaCustomerResponse,msg=string}	"获取单一客户信息,返回包括客户详情"
//	@Router		/bbs/find  [get]
func (api *BbsCategoryApi) FindBbsCategory(c *gin.Context) {
...
func (api *BbsCategoryApi) DeleteById(c *gin.Context) {
	// 绑定参数用来获取/:id这个方式
	id := c.Param("id")
	// 开始执行
...
func (api *BbsCategoryApi) DeleteByIds(c *gin.Context) {
	// 绑定参数用来获取/:id这个方式
	ids := c.Query("ids")
	// 开始执行
...
func (api *BbsCategoryApi) GetBbsCategoryDetail(c *gin.Context) {
	// 绑定参数用来获取/:id这个方式
	id := c.Param("id")
	// 这个是用来获取?age=123
	//age := c.Query("age")
...
// LoadBbsCategoryPage
func (api *BbsCategoryApi) LoadBbsCategoryPage(c *gin.Context) {
	// 创建一个分页对象
	var pageInfo request2.BbsCategoryPageInfo
