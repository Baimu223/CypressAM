...
func (api *XkBbsApi) CreateXkBbs(c *gin.Context) {
	// 1: 第一件事情就准备数据的载体
	var xkBbs bbs.XkBbs
...
func (api *XkBbsApi) UpdateXkBbs(c *gin.Context) {
	// 1: 第一件事情就准备数据的载体
	var xkBbs bbs.XkBbs
	err := c.ShouldBindJSON(&xkBbs)
	if err != nil {
...
func (api *XkBbsApi) GetXkBbs(c *gin.Context) {
	var xkBbs bbs.XkBbs
	// 绑定参数
...
func (api *XkBbsApi) DeleteById(c *gin.Context) {
	// 绑定参数用来获取/:id这个方式
	id := c.Param("id")
	// 开始执行
...
func (api *XkBbsApi) GetXkBbsDetail(c *gin.Context) {
	// 绑定参数用来获取/:id这个方式
	id := c.Param("id")
	// 这个是用来获取?age=123
	//age := c.Query("age")
...
func (api *XkBbsApi) LoadXkBbsPage(c *gin.Context) {
	// 创建一个分页对象
