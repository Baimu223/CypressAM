...
// 保存
func (service *VideoService) SaveVideo(xkVideo *video.XkVideo) (err error) {
	err = global.KSD_DB.Model(xkVideo).Create(xkVideo).Error
	return err
}

// 更新
func (service *VideoService) UpdateVideo(xkVideo *video.XkVideo) (err error) {
	err = global.KSD_DB.Model(xkVideo).Updates(xkVideo).Error
	return err
}

// 根据id查询视频信息
func (service *VideoService) GetXkVideoById(id uint) (xkVideo *video.XkVideo, err error) {
	err = global.KSD_DB.Where("id = ?", id).Find(&xkVideo).Error
	return
}

// 查询所有的视频的数据并分页
func (service *VideoService) FindVideosPage(params *request.XkVideoReq) (resp response.XkVideoResp, err error) {
